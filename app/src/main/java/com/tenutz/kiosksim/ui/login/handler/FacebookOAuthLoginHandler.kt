...
class FacebookOAuthLoginHandler(private val fragment: Fragment) : FacebookCallback<LoginResult> {
    ...
    override fun onSuccess(result: LoginResult) {
        ...
        val graphRequest = GraphRequest.newMeRequest(result.accessToken) { jsonObject, response ->
            viewModel.socialSignupOrLogin(SocialProfileArgs(result.accessToken.token, SocialType.FACEBOOK, jsonObject?.getString("name"), jsonObject?.getString("email"), jsonObject?.getJSONObject("picture")?.getJSONObject("data")?.getString("url")))
        }
